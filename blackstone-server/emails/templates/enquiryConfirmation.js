...
// success message already promises them. Same public, unauthenticated
// submission path as enquiryAdmin.js, so `message` and `name` are escaped
// for the same reason.
export function enquiryConfirmationTemplate({ name, message }) {
  ...
  return layout({
    ...
    bodyHtml: `
      ...
      <p style="margin-top:8px;padding:16px;background-color:#f8f7f4;white-space:pre-wrap;">${safe.message}</p>
    `,
  })
}
